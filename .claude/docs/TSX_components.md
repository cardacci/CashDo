...
Zustand or Redux hooks (`useDispatch`, `useSelector`) for connecting to global state. This section comes before custom hooks because it is more specific to the state management solution and often provides data that custom hooks depend on.
...
A straightforward component that demonstrates the core structure without store or custom hooks. Every block follows alphabetical ordering.
...
A more realistic component that uses store, custom hooks, refs, memos, effects, and multiple function types — showing how all sections work together with full alphabetical ordering applied.
